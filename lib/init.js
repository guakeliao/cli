    ...
    try {
        ...
        if (exists) {
            ...
            inquirer.prompt([{
                type: 'input',
                name: 'version',
                message: 'set a version ?', default: '1.0.0',
            }, {
                type: 'input',
                name: 'description',
                message: 'set a description ?', default: 'welcome to vue',
            }]).then(async (answers) => {
                // Spinner 初始设置
                ...
                // 等待复制好模板文件到对应路径去
                try {
                    await fse.copy(`${templatePath}/template`, targetPath)
                } catch (err) {
                    console.log(symbols.error, chalk.red(`Copy template failed. ${err}`))
                    process.exit()
                }
                // 把要替换的模板字符准备好
                const multiMeta = {name: projectName, version: answers.version, description: answers.description}
                // 把要替换的文件准备好
                const multiFiles = [`${targetPath}/package.json`, `${targetPath}/deploy/package.json`, `${targetPath}/config/webpack.dev.js`, `${targetPath}/src/config/contextPath.js`]
                // 用条件循环把模板字符替换到文件去
